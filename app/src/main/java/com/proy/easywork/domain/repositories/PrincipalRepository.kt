...
class PrincipalRepository(aplication: Application) : BaseRepository(){
    ...
    suspend fun cancelarSolicitudServicio(request: RQClienteCancelarServicio): MADataResult<RSMessage> {
        return try {
            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.clienteCancelarServicio(sp.getToken(), request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body())
                }
