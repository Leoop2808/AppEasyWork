...
class LoginTecnicoRepository(aplication: Application): BaseRepository() {
    ...
    suspend fun authenticateGoogleTecnico(request : RQAuthenticationGoogle): MADataResult<VMAuthentication> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.authenticationGoogleTecnico(basicAuthentication,request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    result.body()?.let {
                        val authentication = it.toAuthentication()
                        ...
                        preferences.saveRol("2")
                        preferences.saveSession(authentication.toUser())
                        Log.e(":)", "toUser $authentication.toUser()")
                    }
                    MADataResult.Success(result.body()?.toAuthentication())
                }
                HttpURLConnection.HTTP_NO_CONTENT -> MADataResult.Failure(
    ...
    suspend fun authenticateFacebookTecnico(request : RQAuthenticationFacebook): MADataResult<VMAuthentication> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.authenticationFacebookTecnico(basicAuthentication,request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    result.body()?.let {
                        val authentication = it.toAuthentication()
    ...
    suspend fun enviarCodigoCelularTecnico(request : RQCodigoCelular): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.enviarCodigoCelularTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
    ...
    suspend fun enviarCodigoCorreoTecnico(request : RQCodigoCorreo): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.enviarCodigoCorreoTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body()?.Message)
                }
    ...
    suspend fun verificarCodigoCorreoTecnico(request : RQVerificarCodigo): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.verificarCodigoCorreoTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body()?.Message)
                }
    ...
    suspend fun verificarCodigoCelularTecnico(request : RQVerificarCodigoCelular): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.verificarCodigoCelularTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body()?.Message)
                }
    ...
    suspend fun actualizarClaveTecnico(request : RQClave): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.actualizarClaveTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body()?.Message)
                }
    ...
    suspend fun enviarCodigoAutenticacionCelularTecnico(request : RQCodigoCelular): MADataResult<String> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.enviarCodigoAutenticacionCelularTecnico(basicAuthentication, request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    MADataResult.Success(result.body()?.Message)
                }
    ...
    suspend fun authenticatePhoneTecnico(request : RQAuthenticationPhone): MADataResult<VMAuthentication> {
        return try {

            val gson = Gson()
            val type = object : TypeToken<RSErrorMessage>() {}.type
            val result = mRemoteClient?.authenticationPhoneTecnico(basicAuthentication,request)
            when (result?.code()) {
                HttpURLConnection.HTTP_OK ->{
                    result.body()?.let {
                        val authentication = it.toAuthentication()
